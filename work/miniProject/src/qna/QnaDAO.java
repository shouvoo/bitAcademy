...
public class QnaDAO {
	...
	public List<QnaDomain> listQna(Page page) throws Exception{
		...
		try {
			...
			StringBuffer sql = new StringBuffer();
			sql.append("select * ")
			.append("from (select rownum rnum , trunc(sysdate - reg_date) as qnabday ,trunc(sysdate - a_reg_date) as aqnabday, a.* ")
			...
			while(rs.next()){
				...
				qna.setQnabDay(rs.getInt("qnabday"));
				qna.setAqnabDay(rs.getInt("aqnabday"));
				list.add(qna);
			}
	...
	public void insertAnswerQna(QnaDomain qna) throws Exception {
		...
		try {
			...
			pstmt.setString(1, qna.getAnComment());
	...
	public QnaDomain detailAnswerQna(int no) throws Exception {
		...
		try {
			con= ConnectionPool.getConnection();
			StringBuffer sql = new StringBuffer();
			sql.append("select a_comment, a_reg_date from t_qboard where no = ?");
			pstmt= con.prepareStatement(sql.toString());
	...
	public void modifyAnswerQna(QnaDomain qna) throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;

...
public class FixDAO {
	...
	public void insertFix(FixVO fix) throws Exception {
		try {			
			...
			//ptmt.setString(index++, fix.getYoutube());
	...
	public List<FixVO> listFix(String order, int page) throws Exception {
		...
		try {
			...
			while(rs.next()) {
				...
				String content = rs.getString("content").split("[☎AIE☎]")[0];
	...
	public void insertReDut(FixVO fix) throws Exception {
		try {
			con = ConnectionPool.getConnection();
			sb = new StringBuffer();
			sb.append("insert into t_fixrecommentboard (");
			sb.append("recomment_no, comment_no, writer, content, id)");
			sb.append("values (");
			sb.append("fixrecommentboard_no.nextval,?,?,?,?)");
			
			ptmt = con.prepareStatement(sb.toString());
			
			int index = 1;
			
			ptmt.setInt(index++, fix.getCommentNo());
			ptmt.setString(index++, fix.getWriter());
			ptmt.setString(index++, fix.getContent());
			ptmt.setString(index++, fix.getId());
	...
	public List<FixVO> detailReDut(int no) throws Exception {
		List<FixVO> list = new ArrayList<>();
		try {
			con = ConnectionPool.getConnection();
			sb = new StringBuffer();
			sb.append("select * ");
			sb.append("from t_fixrecommentboard ");
			sb.append("where comment_no = ? order by recomment_no desc");
			...
			while(rs.next()) {
				...
				dut.setRecommentNo(rs.getInt("recomment_no"));
	...
	public void deleteReDut(int no) throws Exception {
		try {
			con = ConnectionPool.getConnection();
			sb = new StringBuffer();
			sb.append("delete from t_fixrecommentboard where recomment_no = ?");

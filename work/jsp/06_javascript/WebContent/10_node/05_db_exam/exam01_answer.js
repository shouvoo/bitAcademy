...
function detail(request, response) {
	...
	con.query(
		...
		function (err, rows) {
			...
			var board = rows[0];
			fs.readFile("view/board/detail.html", "utf-8", function(err, data) {
				// 오류에 대한 처리 코드 추가 가능
				response.writeHead(200, {"Content-Type": "text/html; charset=utf-8"});
				data = data.replace(":title", board.title);
				data = data.replace(":writer", board.writer);
				data = data.replace(":content", board.content);
				data = data.replace(/:no/g, board.no);
				response.end(data);	
			});	
		}
	);
...
function list(response) {
	...
	con.query(sql, function (err, rows, fields) {
		...
		fs.readFile("view/board/list.html", "utf-8", function(err, data) {
			// 오류에 대한 처리 코드 추가 가능
			response.writeHead(200, {"Content-Type": "text/html; charset=utf-8"});
...
function writeForm(response) {
	fs.readFile("view/board/writeForm.html", "utf-8", function(err, data) {
		// 오류에 대한 처리 코드 추가 가능
		response.writeHead(200, {"Content-Type": "text/html; charset=utf-8"});
		response.end(data);	
	});	
...
function updateForm(request, response) {
	...
	con.query(
		...
		function (err, rows) {
			...
			var board = rows[0];
			fs.readFile("view/board/updateForm.html", "utf-8", function(err, data) {
				// 오류에 대한 처리 코드 추가 가능
				response.writeHead(200, {"Content-Type": "text/html; charset=utf-8"});
				data = data.replace(":title", board.title);
				data = data.replace(":writer", board.writer);

	...
	con.query(
		...
		function (err, rows) {
			...
			fs.readFile("view/board/detail.pug", "utf-8", function(err, data) {
				...
				html = html.replace(":title", board.title);
				html = html.replace(":writer", board.writer);
				html = html.replace(":content", board.content);
				html = html.replace(/:no/g, board.no);
				response.end(html);	
			});	
		}
	);
...
function list(response) {
	...
	con.query(sql, function (err, rows, fields) {
		...
		fs.readFile("view/board/list.pug", "utf-8", function(err, data) {
			var html = pug.render(data, {obj : rows});
			response.writeHead(200, {"Content-Type":"text/html; charset=utf-8"});
			response.end(html);
		});	
	});
	...
	con.query(
		...
		function (err, rows) {
			...
			fs.readFile("view/board/updateForm.pug", "utf-8", function(err, data) {
				// 오류에 대한 처리 코드 추가 가능
				response.writeHead(200, {"Content-Type": "text/html; charset=utf-8"});
				var html = pug.render(data, {board : board});
				response.writeHead(200, {"Content-Type":"text/html; charset=utf-8"});
